...
static void notify_done(redisContext *c, const std::string &vtid, const std::string &pc,
    ...
    if (!r || r->type == REDIS_REPLY_ERROR) {
        std::cerr << "[heap-cpu] notify_done LPUSH failed for " << vtid << ": " << (r ? r->str : "NULL") << "\n";
    }
    REDIS_FREE(r);
    std::cout << "[heap-cpu] done " << vtid << " pc=" << pc << " status=" << status << "\n";
}
...
static void handle_newtensor(LifecycleManager &mgr, const LifecycleCommand &cmd,
    ...
    if (!error.empty()) {
        std::cerr << "[heap-cpu] newtensor error: " << error << "\n";
    ...
    std::cout << "[heap-cpu] newtensor '" << cmd.name << "' → shm=" << shm_name << "\n";
...
static void handle_deltensor(LifecycleManager &mgr, const LifecycleCommand &cmd,
    ...
    if (!error.empty()) {
        std::cerr << "[heap-cpu] deltensor error: " << error << "\n";
    }
    ...
    std::cout << "[heap-cpu] deltensor '" << cmd.name << "'\n";
...
int main(int argc, char **argv) {
    ...
    // 清理上一轮进程残留的过期 registry（堆的生死 = 进程的生死）
    const char *registry_path = "/tmp/deepx_heap_registry.txt";
    unlink(registry_path);

    // 连接 Redis（无限重试，不自退——heap-plat 由元程控制退出）
    redisContext *redis = nullptr;
    while (!redis) {
        redis = connect_redis(redis_addr, redis_port);
        if (!redis) {
            std::cerr << "[heap-cpu] Redis not available, retrying in 1s...\n";
            sleep(1);
        }
    }
    std::cout << "[heap-cpu] connected to Redis " << redis_addr << ":" << redis_port << "\n";
    ...
    std::cout << "[heap-cpu] listening on " << HEAP_QUEUE << " + " << SYS_QUEUE << "\n";
    std::cout << "[heap-cpu] heartbeat → " << HEARTBEAT_KEY << " (every " << HEARTBEAT_INTERVAL_SEC << "s)\n";
    ...
    while (running) {
        ...
        if (!r) {
            ...
            std::cerr << "[heap-cpu] Redis disconnected, reconnecting...\n";
            redisFree(redis);
            redis = nullptr;
            while (!redis) {
                sleep(1);
                redis = connect_redis(redis_addr, redis_port);
                if (!redis) {
                    std::cerr << "[heap-cpu] Redis still not available, retrying...\n";
                }
            }
            register_instance(redis);
        ...
        if (queue_name == SYS_QUEUE) {
            try {
                ...
                if (cmd == "shutdown") {
                    std::cout << "[heap-cpu] received sys shutdown command, exiting...\n";
                    running = false;
                } else {
                    std::cerr << "[heap-cpu] unknown sys command: " << cmd << "\n";
                }
            } catch (const std::exception &e) {
                std::cerr << "[heap-cpu] sys cmd JSON parse error: " << e.what() << "\n";
            }
            continue;
        }
        ...
        try {
            ...
            std::cerr << "[heap-cpu] JSON parse error: " << e.what() << "\n";
            continue;
        }
        ...
        std::cout << "[heap-cpu] received op=" << op << " vtid=" << vtid << " pc=" << pc << "\n";
    ...
    if (redis) {
        ...
        std::cout << "[heap-cpu] final heartbeat: stopped\n";
        redis_cmd(redis, "DEL %s", INSTANCE_KEY);
        redisFree(redis);
    }
    std::cout << "[heap-cpu] shutdown complete.\n";
    return 0;
}
