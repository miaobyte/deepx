...
> exop-metal 的职责边界。哪些能做，哪些**绝对不能碰**。

---

## 1. exop-metal 是什么

在 DeepX 元程 5 核架构中，exop-metal 是**计算平面**的 Metal GPU 实现。

它只做一件事：**被动消费指令 → 执行 GPU kernel → 通知完成**。

exop-metal 是"无状态的 GPU 函数调用器"——它不关心数据从哪来、到哪去、含义是什么。

> **I/O 操作 (print/save/load) 已迁移到 `io-metal` 独立进程。** exop-metal 只处理纯 GPU 计算。
...
| 操作 | 允许 | 说明 |
|------|------|------|
| BLPOP/RPOP `cmd:exop-metal:*` | ✅ | 消费 VM 发来的计算指令 |
...
| SET `/sys/op-plat/exop-metal:0` | ✅ | 启动时注册进程状态 |
| SET NX `/op/exop-metal/*` | ✅ | 首次启动时注册算子列表 |
| DELETE `/sys/op-plat/exop-metal:0` | ✅ | 退出时注销 |
...
> exop-metal 的执行单元是 **tensor 指针 + 元素数量 + dtype**。
...
| **唯一例外**: 启动时的进程状态日志（`[exop-metal] CWD/device/connected/listening`） | ✅ | 一次性输出，帮助排查进程生命周期问题 |
...
```
deepxctl                 VM                    exop-metal               heap-metal
   │                      │                       │                       │
   │ ── SET /vthread/1 ──→│                       │                       │
   │ ── LPUSH notify:vm ─→│                       │                       │
   │                      │── PUSH cmd:exop-metal:0 ─→│                      │
   │                      │                       │── GET /data/x ────→ Redis
   │                      │                       │←── {shm_name,...} ── Redis
...
**exop-metal 的边界：**
- 入: `cmd:exop-metal:*` 队列 + Redis GET（tensor 元信息）
- 出: `done:<vtid>` 队列
- 内部: shm 映射 → GPU 计算 → 返回
